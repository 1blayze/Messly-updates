...
export default function SpotifyCallbackPage() {
  ...
  useEffect(() => {
    ...
    // Browser callback with no active Messly session: handoff to desktop app.
    if (typeof window !== "undefined" && !user && !isLoading) {
      const handoffUrl = buildDesktopHandoffUrl(searchParams);
      setStatus("handoff");
      setMessage("Abrindo o aplicativo Messly para concluir a conexao...");
    ...
    if (!user || isLoading) {
      ...
      setMessage("Aguardando sessao do Messly para concluir a conexao...");
