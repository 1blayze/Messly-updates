...
interface VoiceSignalDedupState {
  sentAt: number;
  action: VoiceCallSignalAction;
}

const VOICE_SIGNAL_ACTION_PRIORITY: Record<VoiceCallSignalAction, number> = {
  invite: 1,
  cancel: 2,
};
const VOICE_SIGNAL_SEND_MAX_ATTEMPTS = 3;
const VOICE_SIGNAL_SEND_RETRY_DELAY_MS = 140;
const VOICE_CALL_START_CONNECT_TIMEOUT_MS = 20_000;
...
function primeAudioCue(refObject: { current: HTMLAudioElement | null }, sourceUrl: string): HTMLAudioElement | null {
  const audioElement = getOrCreateAudioCue(refObject, sourceUrl);
  if (!audioElement) {
    return null;
  }
...
function unlockAudioCuePlayback(audioElement: HTMLAudioElement): void {
...
export default function DirectMessageChatView({
  ...
  const [isChatWindowFocused, setIsChatWindowFocused] = useState<boolean>(() =>
  ...
  const latestVoiceSignalBySenderRef = useRef<Map<string, VoiceSignalDedupState>>(new Map());
  ...
  const isStartingVoiceCallRef = useRef(false);
  ...
  const outgoingVoiceRingAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  const voiceCallStartTimeoutRef = useRef<number | null>(null);
  ...
  const voicePresenceRoomId = useMemo(() => {
    const activeRoomId = String(activeVoiceRoomIdRef.current ?? "").trim();
    if (isVoiceCallActive || isVoiceCallConnecting) {
      return activeRoomId || voiceRoomId;
    }
    return voiceRoomId;
  }, [isVoiceCallActive, isVoiceCallConnecting, voiceRoomId]);
  ...
  useEffect(() => {
    const incoming = primeAudioCue(incomingVoiceRingAudioRef, voiceCallRingingSoundUrl);
    const outgoing = primeAudioCue(outgoingVoiceRingAudioRef, voiceCallRingingSoundUrl);
    const join = primeAudioCue(callJoinAudioRef, voiceCallJoinSoundUrl);
    const leave = primeAudioCue(callLeaveAudioRef, voiceCallLeaveSoundUrl);

    const warmupTargets = [incoming, outgoing, join, leave].filter(
      (audioElement): audioElement is HTMLAudioElement => Boolean(audioElement),
    );
    ...
    const unlockAll = (): void => {
      warmupTargets.forEach((audioElement) => {
        unlockAudioCuePlayback(audioElement);
      });
    };

    window.addEventListener("pointerdown", unlockAll, { once: true, capture: true });
    window.addEventListener("keydown", unlockAll, { once: true, capture: true });
    window.addEventListener("touchstart", unlockAll, { once: true, capture: true });

    return () => {
      window.removeEventListener("pointerdown", unlockAll, true);
      window.removeEventListener("keydown", unlockAll, true);
      window.removeEventListener("touchstart", unlockAll, true);
    };
  }, []);
  ...
  const shouldProcessVoiceSignal = useCallback((
    senderUserIdRaw: string,
    sentAtRaw: number,
    actionRaw: VoiceCallSignalAction,
  ): boolean => {
    ...
    const action: VoiceCallSignalAction = actionRaw === "cancel" ? "cancel" : "invite";
    const latestBySender = latestVoiceSignalBySenderRef.current;
    const known = latestBySender.get(senderUserId);
    if (!known) {
      latestBySender.set(senderUserId, {
        sentAt: sentAtRaw,
        action,
      });
      return true;
    }
    if (sentAtRaw > known.sentAt) {
      latestBySender.set(senderUserId, {
        sentAt: sentAtRaw,
        action,
      });
    ...
    // Same timestamp: allow higher-priority action (cancel beats invite).
    const knownPriority = VOICE_SIGNAL_ACTION_PRIORITY[known.action];
    const incomingPriority = VOICE_SIGNAL_ACTION_PRIORITY[action];
    if (incomingPriority <= knownPriority) {
      return false;
    }
  ...
  const emitVoiceRealtimeEvent = useCallback(
    (eventName: VoiceRealtimeEventName, scopeKeyRaw: string, detail: Record<string, unknown> = {}): void => {
      const scopeKey = String(scopeKeyRaw ?? "").trim();
      const dedupeKey = `${eventName}:${scopeKey || "default"}`;
      const now = Date.now();
      const lastAt = voiceRealtimeEventDedupRef.current.get(dedupeKey) ?? 0;
      if (now - lastAt < 220) {
        return;
      }
      voiceRealtimeEventDedupRef.current.set(dedupeKey, now);

      if (import.meta.env.DEV) {
        console.info(`[voice:event] ${eventName}`, {
          scope: scopeKey,
          ...detail,
        });
      }
      ...
      window.dispatchEvent(
        new CustomEvent("messly:voice-call-event", {
          detail: {
            event: eventName,
            scope: scopeKey,
            at: now,
  ...
  const showWebVoiceNotification = useCallback(
    (titleRaw: string, bodyRaw: string, options?: { tag?: string; icon?: string | null }): void => {
      ...
      void (async () => {
        ...
        const notification = new Notification(title, {
          ...
          tag: String(options?.tag ?? "").trim() || undefined,
          silent: false,
        });
  ...
  const clearVoiceCallStartTimeout = useCallback((): void => {
    if (voiceCallStartTimeoutRef.current != null) {
      window.clearTimeout(voiceCallStartTimeoutRef.current);
      voiceCallStartTimeoutRef.current = null;
    }
  }, []);
  ...
  const notifyIncomingVoiceCallDesktop = useCallback(
    (senderUserIdRaw: string, roomIdRaw: string, sentAtRaw: number): void => {
      ...
      void (async () => {
        let deliveredByDesktopBridge = false;
        if (typeof notifyCall === "function") {
          try {
            const result = await notifyCall({
              conversationId,
              roomId,
              callerUserId,
            ...
            deliveredByDesktopBridge = Boolean(result?.ok);
          } catch {
            deliveredByDesktopBridge = false;
          }
        }

        if (!deliveredByDesktopBridge) {
          showWebVoiceNotification(
            `${safeTargetDisplayName}`,
            "iniciou uma chamada de voz.",
            {
              tag: `voice:incoming:${roomId}`,
              icon: targetAvatarSrc,
            },
          );
        }
      })();
    },
    [conversationId, safeTargetDisplayName, showWebVoiceNotification, targetAvatarSrc, voiceRoomId],
  );
  ...
  const applyIncomingVoiceInviteState = useCallback(
    (senderUserIdRaw: string, roomIdRaw: string, sentAtRaw: number): void => {
      ...
      emitVoiceRealtimeEvent("call:incoming", `${roomId}:${senderUserId}`, {
        roomId,
        senderUserId,
      });
      emitVoiceRealtimeEvent("call:ringing", `${roomId}:${senderUserId}`, {
        roomId,
        senderUserId,
        direction: "incoming",
      });
  ...
  const handleVoicePresenceStateUpdate = useCallback((update: VoiceCallStateUpdate): void => {
    ...
    const remoteParticipants = call.participants.filter((participant) => {
    ...
    const eventName = String(update.event ?? "").trim().toUpperCase();
    const eventScope = `${roomId}:${remoteUserId || "remote"}`;

    if (eventName === "CALL_RINGING") {
      emitVoiceRealtimeEvent("call:ringing", eventScope, {
        roomId,
        remoteUserId,
        remoteDisplayName,
      });
    }
    if (eventName === "CALL_JOINED") {
      emitVoiceRealtimeEvent("call:joined", eventScope, {
        roomId,
        remoteUserId,
        remoteDisplayName,
      });
      if (document.hidden || !document.hasFocus()) {
        showWebVoiceNotification(
          ...
          {
            tag: `voice:joined:${roomId}`,
            icon: targetAvatarSrc,
          },
        );
      }
    }
    if (eventName === "CALL_LEFT") {
      emitVoiceRealtimeEvent("call:left", eventScope, {
        roomId,
        remoteUserId,
        remoteDisplayName,
      });
      if (document.hidden || !document.hasFocus()) {
        showWebVoiceNotification(
          `${remoteDisplayName}`,
          "saiu da chamada de voz.",
          {
            tag: `voice:left:${roomId}`,
            icon: targetAvatarSrc,
          },
        );
      }
    ...
    if (update.event === "CALL_ENDED" && isVoiceCallConnectingRef.current && !hadRemoteParticipantInSessionRef.current) {
      emitVoiceRealtimeEvent("call:ended", `${roomId}:${call.callId}`, {
        roomId,
        callId: call.callId,
      });
      stopAudioCue(outgoingVoiceRingAudioRef, true);
      clearIncomingVoiceInviteState();
    ...
    if (isVoiceCallActiveRef.current || isVoiceCallConnectingRef.current) {
      if (callStatus === "CONNECTED" || callStatus === "RECONNECTING") {
        stopAudioCue(outgoingVoiceRingAudioRef, true);
        emitVoiceRealtimeEvent("call:accepted", `${roomId}:${call.callId}`, {
          roomId,
          callId: call.callId,
        });
      }
    ...
    if (callStatus === "RINGING") {
      ...
      const callerIsActiveRemoteParticipant = remoteParticipants.some(
        (participant) => String(participant.userId ?? "").trim() === callerUserId,
      );
      if (callerUserId && callerUserId !== localUserId && callerIsActiveRemoteParticipant) {
  ...
  const sendVoiceSignal = useCallback(
    async (action: VoiceCallSignalAction, roomIdOverride?: string | null): Promise<void> => {
      ...
      latestVoiceSignalBySenderRef.current.set(senderUserId, {
      ...
      if (action === "cancel") {
        emitVoiceRealtimeEvent("call:ended", `${roomId}:${senderUserId}`, {
          roomId,
          senderUserId,
          reason: "cancelled",
        });
      ...
      const signalClientId = createClientMessageId();
      const waitBeforeRetry = async (attempt: number): Promise<void> => {
        const backoffMs = VOICE_SIGNAL_SEND_RETRY_DELAY_MS * attempt;
        if (backoffMs <= 0) {
          return;
        }
      ...
      for (let attempt = 1; attempt <= VOICE_SIGNAL_SEND_MAX_ATTEMPTS; attempt += 1) {
        if (!activeChannel) {
          break;
        }
        try {
          ...
          realtimeFailure = new Error(`Realtime voice-signal failed with status "${String(realtimeResult)}".`);
        } catch (error) {
          realtimeFailure = error;
        }

        if (realtimeDelivered || attempt >= VOICE_SIGNAL_SEND_MAX_ATTEMPTS) {
          break;
        }
      ...
      for (let attempt = 1; attempt <= VOICE_SIGNAL_SEND_MAX_ATTEMPTS; attempt += 1) {
        try {
          ...
          chatFailure = error;
          if (attempt >= VOICE_SIGNAL_SEND_MAX_ATTEMPTS) {
            break;
          }
          await waitBeforeRetry(attempt);
        }
      ...
      if (!realtimeDelivered && !chatPersisted) {
        const failure =
          chatFailure instanceof Error
            ? chatFailure
            : realtimeFailure instanceof Error
              ? realtimeFailure
              : new Error("Falha ao enviar sinal de voz.");
        reportClientError(failure, {
          scope: "voice.sendSignal",
          action,
          conversationId,
          roomId,
          senderUserId,
        ...
        incrementMetric("voice_signal_send_failure_total", 1);
        throw failure;
      }

      if (!realtimeDelivered || !chatPersisted) {
        incrementMetric("voice_signal_send_partial_total", 1);
        if (import.meta.env.DEV) {
          console.warn("[voice:signal-partial]", {
            action,
            roomId,
            senderUserId,
            realtimeDelivered,
            chatPersisted,
            realtimeFailure:
              realtimeFailure instanceof Error ? realtimeFailure.message : String(realtimeFailure ?? ""),
            chatFailure: chatFailure instanceof Error ? chatFailure.message : String(chatFailure ?? ""),
          });
        }
      } else {
        incrementMetric("voice_signal_send_success_total", 1);
      }
    },
    [conversationId, currentUser.userId, currentUserId, emitVoiceRealtimeEvent, voiceRoomId],
  );
  ...
  const consumeVoiceSignalMessage = useCallback(
    (message: ChatMessageItem): boolean => {
      ...
      if (!shouldProcessVoiceSignal(senderUserId, sentAt, signal.action)) {
        return true;
      }
  ...
  useEffect(() => {
    publishVoiceCallUiSnapshot({
      ...
      conversationId,
      elapsedSeconds: voiceCallStartedAtMs
        ? Math.max(0, Math.floor((Date.now() - voiceCallStartedAtMs) / 1_000))
        : null,
      errorMessage: String(voiceCallError ?? "").trim(),
      microphoneWarning: String(voiceCallMicWarning ?? "").trim(),
    ...
    voiceCallUiState,
    conversationId,
    voiceCallStartedAtMs,
    voiceCallElapsedTick,
    voiceCallError,
    voiceCallMicWarning,
  ...
  useEffect(() => {
    const updateFocusState = (): void => {
      setIsChatWindowFocused(!document.hidden && document.hasFocus());
    };
        ...
        .on(
          ...
          (payload) => {
            ...
            if (action === "invite") {
              ...
              if (!shouldProcessVoiceSignal(senderUserId, sentAt, "invite")) {
                return;
              }
              if (!isVoiceCallActiveRef.current && !isVoiceCallConnectingRef.current) {
            ...
            const sentAt = Number(raw.sentAt ?? 0);
            if (!shouldProcessVoiceSignal(senderUserId, sentAt, "cancel")) {
              return;
            }
  ...
  useEffect(() => {
    ...
    isStartingVoiceCallRef.current = false;
  ...
  useEffect(() => {
    const shouldPlayIncomingRingtone =
      ...
      !isVoiceCallActive &&
      !isVoiceCallConnecting;
    if (!shouldPlayIncomingRingtone || !isChatWindowFocused) {
      stopAudioCue(incomingVoiceRingAudioRef, true);
      return;
    }
  ...
  }, [incomingVoiceInviteFromUserId, isChatWindowFocused, isVoiceCallActive, isVoiceCallConnecting]);
  ...
  const stopVoiceCallSession = useCallback(async (): Promise<void> => {
    ...
    hadRemoteParticipantInSessionRef.current = false;
    connectedRemoteParticipantsCountRef.current = 0;
    emitVoiceRealtimeEvent("call:ended", `${activeRoomId}:local`, {
      roomId: activeRoomId,
      reason: "local-stop",
    });
    if (existingVoiceCallClient) {
  ...
  const startVoiceCallWithRoomId = useCallback((roomIdOverride?: string | null, options?: { suppressInvite?: boolean; origin?: VoiceCallStartOrigin }) => {
    if (
      isStartingVoiceCallRef.current ||
      voiceCallClientRef.current ||
      isVoiceCallActiveRef.current ||
      isVoiceCallConnectingRef.current
    ) {
      return;
    }
    isStartingVoiceCallRef.current = true;
    ...
    const localIdentity: VoiceUserIdentity = {
      userId: String(currentUser.userId ?? "").trim() || String(currentUserId ?? "").trim(),
    ...
    if (!localIdentity.userId) {
      isStartingVoiceCallRef.current = false;
      setVoiceCallError("Nao foi possivel iniciar a chamada. Usuario local invalido.");
      return;
    }
    ...
    if (origin === "outgoing") {
      playAudioCue(outgoingVoiceRingAudioRef, voiceCallRingingSoundUrl, {
        loop: true,
        restart: true,
      });
    }
    ...
    voiceCallStartTimeoutRef.current = window.setTimeout(() => {
      ...
      setVoiceCallError("Tempo limite para conectar a chamada. Tente novamente.");
      void stopVoiceCallSession();
    }, VOICE_CALL_START_CONNECT_TIMEOUT_MS);
    if (origin === "outgoing") {
    ...
    const voiceCallClient = new VoiceCallClient({
      ...
      onParticipantsChanged: (participants) => {
        ...
        const previousRemoteParticipantIds = remoteParticipantIdsRef.current;
        const nextRemoteParticipantIds = new Set(
          normalizedParticipants
            .filter((participant) => !participant.isLocal)
            .map((participant) => participant.userId),
        );
        const joinedRemoteIds = Array.from(nextRemoteParticipantIds).filter((userId) => !previousRemoteParticipantIds.has(userId));
        const leftRemoteIds = Array.from(previousRemoteParticipantIds).filter((userId) => !nextRemoteParticipantIds.has(userId));
        if (joinedRemoteIds.length > 0) {
          playAudioCue(callJoinAudioRef, voiceCallJoinSoundUrl, { restart: true });
        } else if (leftRemoteIds.length > 0 && hadRemoteParticipantInSessionRef.current) {
          playAudioCue(callLeaveAudioRef, voiceCallLeaveSoundUrl, { restart: true });
        }
        joinedRemoteIds.forEach((userId) => {
          emitVoiceRealtimeEvent("call:joined", `${activeVoiceRoomId}:${userId}`, {
            roomId: activeVoiceRoomId,
            userId,
          });
        });
        leftRemoteIds.forEach((userId) => {
          emitVoiceRealtimeEvent("call:left", `${activeVoiceRoomId}:${userId}`, {
            roomId: activeVoiceRoomId,
            userId,
          });
        });
        remoteParticipantIdsRef.current = nextRemoteParticipantIds;
        ...
        const hasRemoteParticipant = nextRemoteParticipantIds.size > 0;
        if (hasRemoteParticipant) {
          stopAudioCue(outgoingVoiceRingAudioRef, true);
          clearVoiceCallStartTimeout();
          emitVoiceRealtimeEvent("call:accepted", `${activeVoiceRoomId}:${remoteIdentity.userId || "remote"}`, {
            roomId: activeVoiceRoomId,
          });
    ...
    void voiceCallClient
      ...
      .catch((error) => {
        ...
        if (origin === "outgoing" && !suppressInviteSignal) {
          const failedRoomId = String(activeVoiceRoomIdRef.current ?? "").trim() || activeVoiceRoomId;
          void sendVoiceSignal("cancel", failedRoomId).catch(() => undefined);
        }
        clearVoiceCallStartTimeout();
        isStartingVoiceCallRef.current = false;
  ...
  const handleAcceptIncomingVoiceInvite = useCallback(() => {
    ...
    const activeRoomId = inviteRoomId || voiceRoomId;
    clearSuppressedIncomingVoiceInvite({
    ...
    stopAudioCue(incomingVoiceRingAudioRef, true);
    stopAudioCue(outgoingVoiceRingAudioRef, true);
    emitVoiceRealtimeEvent("call:accepted", `${activeRoomId}:${incomingVoiceInviteFromUserId || "incoming"}`, {
      roomId: activeRoomId,
      senderUserId: incomingVoiceInviteFromUserId,
    });
    clearIncomingVoiceInviteState();
  ...
  const handleDismissIncomingVoiceInvite = useCallback(() => {
    ...
    stopAudioCue(incomingVoiceRingAudioRef, true);
    emitVoiceRealtimeEvent("call:ended", `${inviteRoomId}:${senderUserId || "incoming"}`, {
      roomId: inviteRoomId,
      ...
      <div className={`dm-chat__body${shouldHideVoiceCallChrome ? " dm-chat__body--voice-call-focus" : ""}`}>
        <div className="dm-chat__main">
          ...
          {visibleMessageRenderEntries.map(({ message, showHeader, mediaGroup, dateDividerLabel }) => {
            ...
            const senderAvatar = String(sender?.avatarSrc ?? "").trim()
              || (isFromCurrentUser ? currentAvatarSrc : targetAvatarSrc);
            const senderFallbackAvatar = getNameAvatarUrl(
              String(sender?.displayName ?? sender?.username ?? "U").trim() || "U",
            );
